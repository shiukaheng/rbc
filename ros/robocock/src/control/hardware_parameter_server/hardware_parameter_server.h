...
class HardwareParameterServer {
    ...
    public:
        HardwareParameterServer() : _nh("hardware_parameter_server") {
            ...
            for (int i = 0; i < NUM_MOTORS; i++) {
                initParam<float>(base_parameters_msg.parameters[i].p_in, i, "p_in", 0.0);
                initParam<float>(base_parameters_msg.parameters[i].i_in, i, "i_in", 0.0);
                initParam<float>(base_parameters_msg.parameters[i].d_in, i, "d_in", 0.0);
                initParam<float>(base_parameters_msg.parameters[i].bias, i, "bias", 0.0);
                ...
                initParam<float>(base_parameters_msg.parameters[i].smoothener_alpha, i, "smoothener_alpha", 0.0);    
